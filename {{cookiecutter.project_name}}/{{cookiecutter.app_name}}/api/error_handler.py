...
def handle_404(error):
    ...
    return return_code.PAGE_NOT_FOUND, 404


@err_bp.app_errorhandler(405)
def handle_405(error):
    return return_code.METHOD_NOT_ALLOWED, 405
...
def handle_500(error):
    ...
    if len(args) == 0:
        ...
        data = error.args
    return return_code.UNKNOWN_ERROR.data(error.args), 500


@err_bp.app_errorhandler(ValidationError)
def handle_validate_error(error):
    logger.exception(error)
    return return_code.PARAM_ILLEGAL.data(error.args), 200
...
def handle_exception(error):
    ...
    if len(args) == 0:
        ...
        data = error.args
    return return_code.UNKNOWN_ERROR.data(data), 500
